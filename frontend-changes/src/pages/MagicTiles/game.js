...
function getNotesInfo(){
    ...
    for(var i=0;i<scaledNotes.length;i++){
...
function geneBlock(){
    ...
    myTiles[myRand] = new Block(myRand, scaledNotes[noteIndex]);
     noteIndex++;
}
...
function Block(index, noteLength){
    if(!eachState[index])
...
function upDate(){//check keyCode whether correct
    ...
    for(i = 0; i < numOfTiles; ++i){
        if(eachState[i]){
            myTiles[i].y += 2;
            // if(myTiles[i].height > 1){
            //     context.fillRect(myTiles[i].x,myTiles[i].y,70,120);
            //     myTiles[i].height = myTiles[i].height-1;
            // }
            // else{ 
            //     context.fillRect(myTiles[i].x,myTiles[i].y,70,120);
